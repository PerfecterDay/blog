...
+ `java.util.Map`
+ `ServletContext`
+ `ServletConfig`（用于访问初始化参数）

等对象都可以作为属性源。

可以查看 `PropertySource` 的类型层级（type hierarchy），了解框架已经提供的具体实现：
...
`PropertySource` 通常不会被单独使用，而是通过 `PropertySources` 对象统一管理。
`PropertySources` 会聚合多个属性源，并结合 `PropertyResolver` 实现，在多个 `PropertySource` 之间按照优先级进行属性查找。

`PropertySource` 的身份标识（identity）并不是由其内部封装的属性内容决定的，而仅仅由 `PropertySource` 的名称（name）决定。

这一点在集合操作场景下非常有用，例如：
+ `MutablePropertySources` 中的相关操作
+ `named(String)` 方法
+ `toString()` 方法
...
#### 注解 @PropertySource
另外，在使用 `@Configuration` 配置类时，可以通过 `@PropertySource` 注解，以一种方便且声明式（declarative）的方式，将属性源添加到当前的 `Environment` 中。
