...
| 前缀         | 示例                          | 说明                                                             |
| ------------ | ----------------------------- | ---------------------------------------------------------------- |
| `classpath:` | `classpath:com/myapp/config.xml` | 从 classpath 加载资源                                            |
| `file:`      | `file:///data/config.xml`     | 以 URL 形式从文件系统加载，参见 `FileSystemResource` Caveats     |
| `https:`     | `https://myserver/logo.png`   | 以 URL 形式加载                                                  |
| (none)       | `/data/config.xml`            | 取决于底层 `ApplicationContext` 的实现                           |
