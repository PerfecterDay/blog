...
Spring Framework 6.x → Spring Boot 3.x → 默认 `IMMEDIATE` ，需要手动配。
Spring Framework 7.x → Spring Boot 4.x → 默认 `GRACEFUL` ，不用配。
...
```
java -jar -Dlogging.level.com.demo.demo=DEBUG -Dspring.config.location=classpath:/configuration/dev/application.yml app.jar
```
