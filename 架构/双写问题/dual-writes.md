...
## [¶](#一-问题本质与难点分析) 一、问题本质与难点分析

### [¶](#h-11-什么是双写问题) 1.1 什么是双写问题
...
### [¶](#h-12-为什么简单的顺序执行不可行) 1.2 为什么简单的顺序执行不可行
...
### [¶](#h-13-真正的难点故障场景下的保证) 1.3 真正的难点：故障场景下的保证
...
## [¶](#三-outbox-模式发件箱模式) 三、Outbox 模式（发件箱模式）
...
### [¶](#h-32-为什么这样能保证一致性) 3.2 为什么这样能保证一致性
...
## [¶](#四-cdcchange-data-capture方案) 四、CDC（Change Data Capture）方案
...
## [¶](#五-结合-outbox-与-cdc-的混合方案) 五、结合 Outbox 与 CDC 的混合方案
...
## [¶](#六-支持事务的消息系统xa-分布式事务) 六、支持事务的消息系统（XA 分布式事务）
...
### [¶](#h-63-已知支持-xa-的消息中间件) 6.3 已知支持 XA 的消息中间件

| 中间件 | XA 支持情况 | 备注 |
| --- | --- | --- |
| **RocketMQ** | ✅ 支持事务消息 | 通过半消息 + 回查机制实现[[7]](#fn7) |
| **endurox** | ✅ 支持 XA | 开源事务处理框架 |
| RabbitMQ | ❌ 不支持 | 官方明确不推荐在 RabbitMQ 中使用 XA[[8]](#fn8) |
| ActiveMQ | ❌ 不推荐 | 性能影响严重 |
| Kafka | ❌ 不支持 | Kafka 设计哲学与 XA 不兼容 |

### [¶](#h-64-为什么不普遍使用) 6.4 为什么不普遍使用
...
## [¶](#七-事件溯源event-sourcing) 七、事件溯源（Event Sourcing）
...
### [¶](#h-73-事件溯源-vs-cdc) 7.3 事件溯源 vs CDC
...
### [¶](#h-74-为什么不只为解决双写而引入) 7.4 为什么不只为解决双写而引入
...
## [¶](#八-其它方案与前沿探索) 八、其它方案与前沿探索

### [¶](#h-81-底层协议强化) 8.1 底层协议强化
...
## [¶](#十-hugo-的工程实践总结) 十、Hugo 的工程实践总结

### [¶](#h-101-内部约定双写问题的处理原则) 10.1 内部约定：双写问题的处理原则
...
8. [Should I use XA](https://activemq.apache.org/should-i-use-xa) — Apache ActiveMQ 官方文档 [↩︎](#fnref8) [↩︎](#fnref8:1)
