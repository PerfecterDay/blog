...
注意与 `export x=1` 区分，`export` 是将变量导出为环境变量，子进程可以使用。
...
第三个参数: 123
```
注意 `${10}`-第10个参数（必须加大括号）, `$#` 可以获取参数数量。

`$@`获取所有参数， 遍历参数，创建以下脚本 test.sh：
```
#!/bin/bash
...
运行 `./test.sh aaa "hello world" ccc` 会输出：
```
参数: aaa
参数: hello world
参数: ccc
