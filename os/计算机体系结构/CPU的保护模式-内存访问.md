...
<center>
	<img src="pics/segment_selector.png" width="50%">
</center>
...
CR3 保存的是物理地址，如果 CR3 本身又是一个虚拟地址，那么 CPU 得先： `虚拟地址 → 查页表 → 物理地址` 才能找到“查页表用的页表”。这就形成鸡生蛋的问题。所以：
CR3 指向的顶级页表必须能够直接被硬件定位，因此 CR3 保存的是顶级页表的物理地址。
