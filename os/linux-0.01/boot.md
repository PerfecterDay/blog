...
这段定义了一个 GDT（全局描述符表），里面有 3 个 8 字节的描述符。我把它们逐字节、逐位翻译成内存中的真实样子。
三个描述符在内存中的字节
源码里每个描述符用 4 个  .word  写成。以代码段为例：
```
gdt:
	.word	0,0,0,0		; dummy
	...
	.word	0x00C0		; granularity=4096, 386
```
按小端展开成 8 个字节（内存地址从低到高）：

选择子	描述符	内存字节（低→高）
0x00	空描述符 dummy	 00 00 00 00 00 00 00 00 
0x08	代码段	 FF 07 00 00 00 9A C0 00 
0x10	数据段	 FF 07 00 00 00 92 C0 00 
（选择子 = 索引×8：第 1 个描述符=0x08，第 2 个=0x10，正好对应 head.s 里  movl $0x10  装 ds/es 用的就是这个数据段。）

逐位解读代码段  FF 07 00 00 00 9A C0 00 
+ Limit = 字节0-1  07FF  + 字节6低4位  0  =  0x0_07FF 
+ Base = 字节2-3  0000  + 字节4  00  + 字节7  00  =  0x00000000 
+ 访问字节 =  0x9A  =  1001 1010 
  + P=1（存在）
  + DPL=00（ring 0，内核态）
  + S=1（代码/数据段，非系统段）
  + Type= 1010 （可执行、可读的代码段）
+ 标志半字节 = 字节6高4位  0xC  =  1100 
  + G=1（粒度 4KB，界限按 4KB 为单位）
  + D/B=1（32 位段）
  + L=0、AVL=0
所以段大小 = (Limit+1)×4KB = (0x7FF+1)×4096 = 2048×4096 = 8MB，基址 0，32 位可执行段——正是注释说的「8Mb / code read/exec / granularity=4096, 386」
